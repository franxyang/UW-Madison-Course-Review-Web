...
- [ ] Vercel 项目创建 + GitHub 连接
- [ ] 环境变量配置 (DATABASE_URL, NEXTAUTH_SECRET, Google OAuth, Redis)
- [ ] 域名配置
- [ ] Neon DB 分支 (main/dev)
- [ ] CI/CD 验证 (push → 自动构建部署)
- [ ] GitHub repo 转 private
...
### 4A: 内容审核 (3-4天)
- [ ] adminProcedure middleware + Admin Layout
- [ ] 举报队列 (/admin/reports) — 批量处理
- [ ] Review 管理 (/admin/reviews) — 搜索/删除/编辑
- [ ] Schema: AuditLog + Report 扩展字段

### 4B: 用户管理 (2-3天)
- [ ] 用户列表 (/admin/users) — 搜索/筛选/角色
- [ ] 封禁系统 — 临时/永久 + 自动解封
- [ ] 角色管理 — STUDENT/MODERATOR/ADMIN 三级
- [ ] Schema: UserBan + UserRole 扩展

### 4C: 系统监控 (2天)
- [ ] Dashboard (/admin) — 统计总览 + 7日趋势
- [ ] 操作日志 (/admin/logs) — 审计追溯
- [ ] AuditLog 自动记录所有 admin 操作

### 4D: 数据管理 (上线后迭代)
- [ ] 课程批量导入 / 学期同步 / Instructor 编辑
